...
- If `decorrelate` is non-empty and `decor_lambda > 0`, `train.py` trains in two stages. In BDT mode, stage 1 uses native `multi:softprob` without decorrelation and stage 2 resumes from the saved stage-1 best model for `n_estimators_decorr` additional rounds with the custom multiclass objective. In NN mode, stage 1 trains a weighted PyTorch MLP and stage 2 resumes from the saved stage-1 best MLP for `epochs_decorr` epochs with the smooth-CvM decorrelation term added to the mini-batch objective. Both modes choose stage-1 best on test `classification_loss` and stage-2 best on test `total_loss`; if decorrelation is disabled, stage 2 is skipped and the stage-1 model is promoted to the main output path.
- In both stages and both model types, `classification_loss` is the same sum-scale weighted softprob loss built from the class-balanced unit-scale training weights (`Σ w·(-log p_true)`), while `mlogloss` is the weighted average of that same quantity (`Σ w·(-log p_true) / Σ w`). BDT stage 1 receives transformed probabilities from XGBoost's builtin objective and BDT stage 2 receives raw margins from the custom objective; NN stages evaluate the same formulas on PyTorch logits after an explicit multiclass softmax. This keeps the stage-1 and stage-2 logged `classification_loss` directly comparable while retaining `mlogloss` as the weighted-average reference metric.
- In BDT mode, `regularization_loss` remains a diagnostic quantity and is reconstructed from the current trees in the same native tree-solver parameterization as XGBoost: `gamma` times the number of leaves, plus the `reg_lambda` / `reg_alpha` penalties evaluated on the unshrunk leaf weights (`leaf / eta` for the stage that produced that boosting round). In NN mode, optimizer `weight_decay` is applied during training but tree regularization diagnostics are recorded as zero. `total_loss = classification_loss + decorrelation_loss` is saved for comparison plots, while `regularization_loss` stays separate. Stage 1 uses `classification_loss` and stage 2 uses `total_loss` for early stopping and best-round selection; `regularization_loss` is not used for either.
