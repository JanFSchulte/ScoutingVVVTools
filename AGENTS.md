...
- `train.py` builds two threshold views when a configured `decorrelate` branch also appears in `selection.json` thresholds. The training view removes only those decorrelate-branch threshold conditions and is used for model training, early stopping, decorrelation-loss state, and the training/test class balancing. The full evaluation view reapplies the omitted decorrelate-branch thresholds on top of the training view and is used for ordinary branch distributions, ROC, score distributions, feature importance, `feature_corr.pdf`, and `test_reference_signal_region.npz`.
