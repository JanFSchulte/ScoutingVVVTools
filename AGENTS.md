...
- In the custom-objective decorrelation modes (`smooth_cvm` and `cvm`), the BDT decorrelation term is multiplied by one fixed train-split scale calibrated from the saved stage-1 best model, using `(cls_loss_stage1 + reg_loss_stage1) / max(raw_decor_loss_stage1, ε)`. NN mode supports `smooth_cvm` and calibrates with the stage-1 mean-CE loss, `cls_loss_stage1 / max(raw_decor_loss_stage1, ε)`, where `raw_decor_loss_stage1` is the same mini-batch smooth-CvM objective used by NN training. This keeps `decor_lambda` controlling the decorrelation term relative to the same loss definition used by NN stage 1. `smooth_cvm` is the default differentiable mode: it matches the old CvM objective conceptually by comparing class-wise score-efficiency curves across decorrelation-variable bins, but replaces the hard bin/CDF steps with smooth sigmoid surrogates so the recorded decorrelation loss, total loss, and training gradient all refer to the same smooth objective. `cvm` keeps the original hard-bin CvM-style loss and its legacy surrogate gradient for backward-compatible BDT studies. Early stopping remains stage-local, using test `classification_loss` for BDT stage 1, test `mlogloss` for NN stage 1, and test `total_loss` in stage 2.
