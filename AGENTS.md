...
Model-dependent comparison plots are saved twice with `_cls` / `_decorr` suffixes where applicable: `importance_cls.pdf` / `importance_decorr.pdf`, `roc_*_cls.pdf` / `roc_*_decorr.pdf`, `score_*_cls.pdf` / `score_*_decorr.pdf`, `decor_corr_{train,test}_cls.pdf` / `decor_corr_{train,test}_decorr.pdf`, `decor_score_vs_branch_cls.pdf` / `decor_score_vs_branch_decorr.pdf`, and `decor_branch_shapes_by_signal_score_cls.pdf` / `decor_branch_shapes_by_signal_score_decorr.pdf`. BDT importance uses XGBoost gain; NN importance keeps the same PDF names but uses deterministic permutation importance on the configured number of test events. The two decorrelation diagnostics are multipage PDFs built from test events: score-vs-branch pages use the matching class's test sample with a black weighted-average score curve over fine branch-value bins, while signal-score-shape pages compare normalized decorrelate-branch distributions after weighted-efficiency cuts from each tree's `decor_efficiencies`. Both use decorrelate branch values after threshold filtering and clipping but before log transform, and normalized plots use `A.U.` on the y-axis. `feature_corr.pdf`, BDT `loss_mlogloss.pdf` / `loss_classification.pdf` or NN `loss_weighted_ce.pdf`, plus `loss_decorrelation.pdf` and `loss_total.pdf` are saved once, and the loss plots draw a vertical line at the stage boundary with NN x-axes labelled by epoch and BDT x-axes by boosting round. During training, BDT stage 1 prints train/test `mlogloss`; NN stage 1 prints `train-update-weighted_ce_loss`, `train-eval-weighted_ce_loss`, and `test-eval-weighted_ce_loss`; NN stage 2 prints those same update/eval diagnostics plus decorrelation and total losses, while BDT stage 2 keeps the legacy `mlogloss`, `classification_loss`, `decorrelation_loss`, and `total_loss` fields. In BDT mode, `classification_loss` is the sum-scale weighted softprob cross-entropy (`Σ w·(-log p_true)`), `mlogloss` is the weighted average of that same quantity (`Σ w·(-log p_true) / Σ w`), and `total_loss = classification_loss + decorrelation_loss`; BDT mode additionally reconstructs `regularization_loss` per newly added tree using that tree's current `eta` together with `gamma` / `reg_lambda` / `reg_alpha`. In NN mode, the update diagnostic is the actual PyTorch train-mode mini-batch weighted CE used for `backward()`, while the plotted train/test `weighted_ce_loss` and the stage-1 early-stopping metric are eval-mode full-split `Σ(w·CE)/Σw`; `total_loss = weighted_ce_loss + decorrelation_loss`, and optimizer `weight_decay` remains native AdamW behavior rather than a separate logged loss term. The decorrelation loss itself uses the current event weights in smooth-CvM mode for NN and in smooth/hard CvM modes for BDT. Smooth-CvM state is prepared per class so zero-weight off-class entries are not recomputed every round, and metric-only loss evaluation skips gradient/Hessian work while keeping the same recorded loss. BDT stage 1 chooses early stopping / best round on test `classification_loss`; NN stage 1 uses full-split test `weighted_ce_loss`; stage 2 uses test `total_loss` in both modes. The fixed decorrelation scale is calibrated once from the saved stage-1 best model as `(cls_loss_stage1 + reg_loss_stage1) / max(raw_decor_loss_stage1, ε)` for BDT and full-split eval weighted CE `cls_loss_stage1 / max(raw_decor_loss_stage1, ε)` for NN, with NN using the same mini-batch smooth-CvM raw loss used during training. The saved reference files keep the pre-lumi physics test weights (`σ × N_tree / N_raw` with the same per-event raw-w shaping as the downstream scripts), and downstream comparison multiplies those stored weights by the local analysis `lumi` before checking them against the reconstructed physical weights.
...
- Loads the BDT-tree copies of `config.json` (to get `class_groups`, `input_root`, `input_pattern`), `branch.json` (to get model input branches), `selection.json` (for `clip_ranges`, `thresholds`, `log_transform`), and `test_ranges.json` from the directory resolved by `bdt_root` with `{tree_name}` substituted. `input_root` is resolved relative to the BDT script directory (the parent of `bdt_root`), matching `train.py`'s convention.
- Discovers plottable variables from `selections/convert/branch.json`'s output entries for the tree (only `onlyMC: false`, expanding `slots` into `{name}_1 … {name}_N`, minus that tree's `skip_branches` from `plotting/branch.json`) and appends one derived model score branch per class named `score_{class_name}` unless skipped.
...
- For each data sample in `data_samples`, loads every event with `weight = 1` per event. Data samples do not read the reweight branches. Data and MC both use the configured trained-model input pattern; missing sample files are treated as fatal errors rather than falling back to another directory.
...
- For derived score branches only, MC is loaded from the saved test split in `test_ranges.json`, weighted per sample to the same full-sample target total `lumi_total × xsection × tree_entries_total / raw_entries` using the raw-weight sum over the loaded test split, filtered with the full BDT `thresholds`, standardised with clip/log transforms, predicted with the saved model, and validated against `test_reference_signal_region.npz` after multiplying the reference weights by `lumi_total`. Data score branches are predicted from the full configured `data_samples` input with unit data weights.
...
- **[selections/BDT/config.json](selections/BDT/config.json)** — controls model training inputs and outputs: `submit_trees`, `input_root`, `input_pattern`, `class_groups`, `output_root`, `model_pattern`, top-level `model_type` (`bdt` or `nn`, defaulting to `bdt`), `entries_per_sample`, `train_fraction`, top-level `decor_loss_mode` (`smooth_cvm` by default, or legacy `cvm` for BDT), `decor_lambda`, `decor_n_bins`, `decor_n_thresholds`, `decor_score_tau`, `decor_bin_tau_scale`, and per-tree BDT hyperparameters (`n_estimators`, `n_estimators_decorr`, `max_depth`, `learning_rate`, `learning_rate_decorr`, optional `min_learning_rate` / `lr_reduce_patience`, `gamma`, `reg_lambda`, `reg_alpha`, `min_child_weight`, `subsample`, `colsample_bytree`, optional `colsample_bynode`, `early_stopping_rounds` with default 10) plus `decorrelate`, `decor_efficiencies` (fractions in `(0, 1]` for the decorrelation shape diagnostics), and `event_reweight_branches` (list of per-event reweight branches multiplied together before sample normalisation; default `["weight_pu"]`). In NN mode, `{tree}_nn` blocks (`fat2_nn`, `fat3_nn`) provide PyTorch MLP hyperparameters: `epochs`, `epochs_decorr`, `hidden_layers`, `activation`, `dropout`, `batch_norm`, `batch_size`, `learning_rate`, `learning_rate_decorr`, optional `min_learning_rate` / `lr_reduce_patience`, `weight_decay`, `early_stopping_rounds`, and `permutation_importance_events`; the checked-in NN blocks set `batch_norm: false` so weighted-event training is not coupled to unweighted BatchNorm running statistics.
...
- In the custom-objective decorrelation modes (`smooth_cvm` and `cvm`), the BDT decorrelation term is multiplied by one fixed train-split scale calibrated from the saved stage-1 best model, using `(cls_loss_stage1 + reg_loss_stage1) / max(raw_decor_loss_stage1, ε)`. NN mode supports `smooth_cvm` and calibrates with the stage-1 full-split eval weighted CE, `cls_loss_stage1 / max(raw_decor_loss_stage1, ε)`, where `raw_decor_loss_stage1` is the same mini-batch smooth-CvM objective used by NN training. This keeps `decor_lambda` controlling the decorrelation term relative to the same loss definition used by NN validation. `smooth_cvm` is the default differentiable mode: it matches the old CvM objective conceptually by comparing class-wise score-efficiency curves across decorrelation-variable bins, but replaces the hard bin/CDF steps with smooth sigmoid surrogates so the recorded decorrelation loss, total loss, and training gradient all refer to the same smooth objective. `cvm` keeps the original hard-bin CvM-style loss and its legacy surrogate gradient for backward-compatible BDT studies. Early stopping remains stage-local, using test `classification_loss` for BDT stage 1, test `weighted_ce_loss` for NN stage 1, and test `total_loss` in stage 2.
...
- After writing the final model, `train.py` also writes `test_reference_signal_region.npz`, `test_reference_qcd_est.npz`, and `test_reference_qcd_est_full.npz`. Each file stores the exact test-split feature order, sample order, class labels, pre-lumi physics weights, predicted class probabilities, and per-array tolerances for the corresponding downstream preprocessing chain. `signal_region.py`, `qcd_est.py`, and the derived-score path in `plotting/data_mc.py` multiply the stored reference weights by their local configured luminosity before comparing them to the reconstructed physical weights; any remaining mismatch is treated as fatal and the script exits instead of silently proceeding.
