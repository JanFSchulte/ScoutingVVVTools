...
Downstream scripts — `selections/BDT/train.py`, `selections/BDT/signal_region/signal_region.py`, and `plotting/data_mc.py` — all consume these files via their config's `input_root` + `input_pattern` (`{input_root}/{sample_group}/{sample}.root`). They discover every file for a given sample by globbing the base pattern **and** the `{sample}_*.root` split variant, then concatenate their trees in sorted order. The convert expression engine supports collection summaries such as `sum`, `max_value`, `min_value`, `nth_max_value(collection, expr, rank, default)`, `value_at_max(collection, key_expr, value_expr, default)`, and `value_at_nth_max(collection, key_expr, value_expr, rank, default)`. `selections/convert/branch.json` writes AK4 b-tag summary outputs for both `fat2` and `fat3`: `ScoutingPFJetRecluster2_scoutUParT_probb_max`, `ScoutingPFJetRecluster2_scoutUParT_probb_max_pt`, `ScoutingPFJetRecluster2_scoutUParT_probb_sum`, `ScoutingPFJetRecluster2_scoutUParT_probb_second_max`, and `ScoutingPFJetRecluster2_scoutUParT_probb_second_max_pt`. The BDT training branch list is independent and does not automatically consume newly produced convert branches.
...
- Discovers plottable variables from `selections/convert/branch.json`'s output entries for the tree (only `onlyMC: false`, expanding `slots` into `{name}_1 … {name}_N`, minus that tree's `skip_branches` from `plotting/branch.json`) and appends one derived BDT score branch per class named `score_{class_name}` unless skipped.
